...
class file_handler:
    ...
    def unshuffle_index(i:int, seed:int, datalen:int) -> int:
        si = pd.DataFrame(data=[i for i in range(datalen)]).sample(frac=1, random_state=seed).values.tolist()
        return si[i][0]

    
    @staticmethod
    def unshuffle_dataframe(df:pd.DataFrame, seed:int) -> pd.DataFrame:
        si = [i[0] for i in pd.DataFrame(data=[i for i in range(df.shape[0])]).sample(frac=1, random_state=seed).values.tolist()]
