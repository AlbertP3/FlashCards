...
class db_queries:
    ...
    def get_unique_signatures_with_existing_files(self) -> set:
        return set(self.db['SIGNATURE'].unique()).intersection(fd.signature for fd in self.files.values() if fd.kind=='revision')
    ...
    def get_chart_positives(self, signature=None) -> list[int]:
        return self.__get_chart_data_deduplicated('POSITIVES', signature).values.tolist()
    ...
    def get_chart_time_spent(self, signature=None) -> list[int]:
        return self.__get_chart_data_deduplicated('SEC_SPENT', signature).values.tolist()
        
    def __get_chart_data_deduplicated(self, return_col_name, signature=None) -> pd.DataFrame:
